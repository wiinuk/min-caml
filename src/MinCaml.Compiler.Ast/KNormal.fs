...
let rec g env = function
    ...
    | Syntax.Neg e -> insertLet (g env e) <| fun x -> Neg x, Type.Int

    // 足し算のK正規化
    | Syntax.Add(e1, e2) ->
        insertLet (g env e1) <| fun x ->
        insertLet (g env e2) <| fun y ->
        Add(x, y), Type.Int
    ...
    | Syntax.FNeg e -> insertLet (g env e) <| fun x -> FNeg x, Type.Float
    | Syntax.FAdd(e1, e2) ->
        insertLet (g env e1) <| fun x ->
        insertLet (g env e2) <| fun y ->
        FAdd(x, y), Type.Float
    ...
    | Syntax.Let((x, t), e1, e2) ->
        let e1', _ = g env e1
        let e2', t2 = g (Map.add x t env) e2
        Let((x, t), e1', e2'), t2
    ...
    | Syntax.Var x ->
        ...
        | _ -> failwithf "external variable %s does not have an array type" x

    | Syntax.LetRec({ name = x, t; args = yts; body = e1 }, e2) ->
        let env' = Map.add x t env
        let e2', t2 = g env' e2
        let e1', _ = g (Map.addList yts env') e1
    ...
    | Syntax.App(Syntax.Var f, e2s) when not <| Map.containsKey f env ->
        ...
        | Type.Fun(_, t) ->
            ...
            let rec bind xs = function
                | [] -> ExtFunApp(f, xs), t
                | e2::e2s -> insertLet (g env e2) <| fun x -> bind (xs @ [x]) e2s
    ...
    | Syntax.Tuple es ->
        ...
        let rec bind xs ts = function
            ...
            insertLet g_e <| fun x -> bind (xs @ [x]) (ts @ [t]) es

        bind [] [] es

    | Syntax.LetTuple(xts, e1, e2) ->
        insertLet (g env e1) <| fun y ->
        let e2', t2 = g (Map.addList xts env) e2
        LetTuple(xts, y, e2'), t2

    | Syntax.Array(e1, e2) ->
        insertLet (g env e1) <| fun x ->
        let _, t2 as g_e2 = g env e2
        insertLet g_e2 <| fun y ->
            let l =
                match t2 with
